...
int GoForIt(TPRJ* Prj) {
	...
	if (Lang == "C") {
		...
		std::string tabs{ "" };
		std::string src{ "// This code is generated by QVersion! It's best NOT to manutally alter it!\n\n" };
		auto spaces{ Prj->data->List("Project", "NameSpaces") };
		for (auto& ns : *spaces) {
			src += tabs + "namespace " + ns + " {\n";
			tabs += "\t";
		}
		src += tabs + "public class QVersionClass {\n";
		src += tabs + "\treadonly public uint giant=0, major=0, minor=0; \n";
		src += tabs + "\treadonly public string dt=\"00000000000000\";\n";
		src += tabs + "\treadonly public string Status=\"Stable\";\n";
		src += tabs + "\tprivate string _Version(bool full=true) {\n";
		src += tabs + "\t\tstring ret=$\"{giant}.{major}.{minor}\";\n";
		src += tabs + "\t\tif (full) ret+=$\".{dt}\";\n";
		src += tabs + "\t\tif (Status!=\"Stable\") ret += $\" ({Status})\";\n";
		src += tabs + "\t\treturn ret;\n";
		src += tabs + "\t}\n";
		src += tabs + "\tpublic string Version => _Version(false);\n";
		src += tabs + "\tpublic string FullVersion => _Version(true);\n";
		src += tabs + "\toverride public string ToString() => Version;\n";
		//src += tabs + "\tinline QVersionClass(){} // Fuck you!\n";
		...
		if (Prj->data->Value("Version", "Status") == "") Prj->data->Value("Version", "Status", "WIP/In development");
		src += tabs + TrSPrintF("\treadonly public static QVersionClass QVersion = new QVersionClass(%d,%d,%d,\"%s\",\"%s\");\n\n", gi, ma, mi, dt.c_str(), Prj->data->Value("Version", "Status").c_str());
